 ...
 *
 * Ported from 4.19: drivers/soc/oplus/system/oplus_project/qcom/oplus_project.c
 * 5.10 adaptations:
 *   - struct file_operations → struct proc_ops
 *   - get_fs()/set_fs() removed (set_fs() deleted in 5.10)
 *   - EXPORT_SYMBOL → EXPORT_SYMBOL_GPL
 *   - Removed dead #include <linux/uaccess.h>
 */
...
static ProjectInfoOCDT *g_project;
...
struct proc_dir_entry *oplus_info;
...
/**
 * get_project() - Get the OPLUS project identifier from SMEM OCDT
 *
 * Reads the current project number from SMEM (item 135, BCDT section)
 * and returns it.  Called widely from charging, display, audio, and
 * touchscreen drivers to determine per-project configuration.
 *
 * Context: any (function is self-initialising on first call)
 * Return:  project number from SMEM, 0 if SMEM unavailable
 */
...
#define RPMB_KEY_PROVISIONED 0x00780178
	...
	static unsigned int rpmb_enable;
...
EXPORT_SYMBOL_GPL(rpmb_is_enable);

/**
 * get_eng_version() - Get the engineering version from SMEM ECDT
 *
 * Reads the engineering version field from the SMEM Extended OCDT
 * (ECDT section).  Used by oplus_daily_build() and test-configuration
 * code to determine the build variant (CMCC / CU / CT / etc.).
 *
 * Context: any
 * Return:  eng version number, -EINVAL if SMEM unavailable
 */
...
EXPORT_SYMBOL_GPL(oplus_daily_build);

bool is_confidential(void)
{
	init_project_version();

	return g_project ? g_project->nDataECDT.Is_confidential : false;
}
EXPORT_SYMBOL_GPL(is_confidential);
...
EXPORT_SYMBOL_GPL(get_oplus_feature);
...
EXPORT_SYMBOL_GPL(get_serialID);
...
/*
 * 5.10: get_fs()/set_fs() removed — proc_symlink() no longer needs
 * address-space override, the kernel always uses KERNEL_DS natively.
 */
...
static int __init oplus_project_init(void)
{
	...
	p_entry = proc_create_data("prjName", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(PROJECT_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("pcbVersion", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(PCB_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("oplusBootmode", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(OPLUS_BOOTMODE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("RFType", 0444, oplus_info,
				  &project_info_fops, UINT2Ptr(RF_INFO));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("modemType", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(MODEM_TYPE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("operatorName", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(OPERATOR_NAME));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("ocp", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(OCP_NUMBER));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("serialID", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(SERIAL_NUMBER));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("engVersion", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(ENG_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("isConfidential", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(CONFIDENTIAL_STATUS));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("cdt", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(CDT_INTEGRITY));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("feature", 0444, oplus_info,
				  &project_info_fops,
				  UINT2Ptr(OPLUS_FEATURE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("test", 0444, oplus_info,
				  &project_info_fops,
