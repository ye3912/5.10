...
static struct clk_regmap *npu_qdsp6ss_sm8250_clocks[] = {
	[NPU_DSP_CORE_CLK_SRC] = &npu_dsp_core_clk_src.clkr,
};

static struct clk_regmap *npu_qdsp6ss_pll_sm8250_clocks[] = {
...
static const struct regmap_config npu_qdsp6ss_sm8250_regmap_config = {
	.name = "qdsp6ss",
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct regmap_config npu_qdsp6ss_pll_sm8250_regmap_config = {
	.name = "qdsp6ss_pll",
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc npu_qdsp6ss_sm8250_desc = {
	.config = &npu_qdsp6ss_sm8250_regmap_config,
	.clks = npu_qdsp6ss_sm8250_clocks,
	.num_clks = ARRAY_SIZE(npu_qdsp6ss_sm8250_clocks),
};

static const struct qcom_cc_desc npu_qdsp6ss_pll_sm8250_desc = {
	.config = &npu_qdsp6ss_pll_sm8250_regmap_config,
	.clks = npu_qdsp6ss_pll_sm8250_clocks,
	.num_clks = ARRAY_SIZE(npu_qdsp6ss_pll_sm8250_clocks),
};

static const struct of_device_id npu_cc_sm8250_match_table[] = {
	{ .compatible = "qcom,npucc-sm8250" },
	{ .compatible = "qcom,npucc-sm8250-v2" },
	{ }
};

static void npu_cc_sm8250_fixup_konav2(struct regmap *regmap)
{
...
static int npu_cc_sm8250_fixup(struct platform_device *pdev,
	struct regmap *regmap)
{
	...
	if (!strcmp(compat, "qcom,npucc-sm8250-v2"))
		npu_cc_sm8250_fixup_konav2(regmap);

	return 0;
}

static int npu_clocks_sm8250_probe(struct platform_device *pdev,
				 const struct qcom_cc_desc *desc)
{
	...
	if (!strcmp("cc", desc->config->name)) {
		...
		ret = npu_cc_sm8250_fixup(pdev, regmap);
		if (ret)
			return ret;
...
static int npu_cc_sm8250_probe(struct platform_device *pdev)
{
	...
	ret = npu_clocks_sm8250_probe(pdev, &npu_cc_sm8250_desc);
	if (ret < 0) {
	...
	ret = npu_clocks_sm8250_probe(pdev, &npu_qdsp6ss_sm8250_desc);
	if (ret < 0) {
	...
	ret = npu_clocks_sm8250_probe(pdev, &npu_qdsp6ss_pll_sm8250_desc);
	if (ret < 0) {
...
MODULE_DESCRIPTION("QTI NPUCC SM8250 Driver");
