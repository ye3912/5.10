...
static void ufs_qcom_cpufreq_dwork(struct work_struct *work)
{
	...
	dev_dbg(host->hba->dev, "cur_freq_vote=%d,freq_val=%u,cth=%lu\n",
...
static int add_group_qos(struct qos_cpu_group *qcg, enum constraint type)
{
	...
	for_each_cpu(cpu, &qcg->mask) {
		dev_dbg(qcg->host->hba->dev, "%s: cpu: %d | mask: %*pb | qos_req: %p\n",
...
static int __ufs_qcom_set_bus_vote(struct ufs_qcom_host *host, int vote)
{
	...
	if (err) {
		dev_err(dev, "Error: failed setting %s bus vote, err=%d\n",
			UFS_DDR, err);
		return err;
	}
...
static void ufs_qcom_qos(struct ufs_hba *hba, int tag, bool is_scsi_cmd)
{
	...
	if (qcg->voted) {
		dev_dbg(qcg->host->hba->dev, "%s: qcg: %p | mask: %*pb - Already voted - return\n",
...
static int ufs_qcom_setup_qos(struct ufs_hba *hba)
{
	...
	for (i = 0; i < qr->num_groups; i++, qcg++) {
		...
		dev_dbg(dev, "%s: qcg: %p | mask: %*pb | mask-wt: %d | qos_req: %p\n",
		...
		policy = cpufreq_cpu_get(cpu);
		if (!policy) {
			dev_err(dev, "%s: Failed cpufreq policy, cpu=%d\n",
				__func__, cpu);
...
static int ufs_qcom_override_clk_freq(struct ufs_hba *hba)
{
 ...
 out:
	if (ret) {
		dev_err(hba->dev, "%s: %s clk set rate(%luHz) failed, %d\n",
				__func__, clki->name,
				max_freq, ret);
	}
	return ret;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf,
		"Clk_mode: %s ,turbo_enabled: %d, axi_freq:%u,unipro_freq:%u, ice_freq: %u\n",
...
static int ufs_cpufreq_status(void)
{
	...
	if (!policy) {
		pr_warn("ufs-qcom: cpufreq not probed yet, defer once\n");
		return -EPROBE_DEFER;
	}
