...
/**
 * dpu_sde_kms_init - Initialize the downstream flat SDE KMS backend
 * @ddev:        Pointer to DRM device
 * @sde_pdev:    Pointer to SDE platform device (qcom,sde-kms)
 *
 * This function allocates and initializes a dpu_kms structure for the
 * downstream flat SDE DT layout.  The SDE DT uses flat reg-names
 * (mdp_phys, vbif_phys, vbif_nrt_phys, regdma_phys) instead of the
 * upstream DPU DT layout (mdp, vbif, vbif_nrt, regdma).
 *
 * The hw_init callback (dpu_sde_kms_hw_init) maps the flat reg-names
 * and creates a stub DSI connector when no real DSI bridge is bound,
 * ensuring at least one encoder/CRTC pair exists for DRM object init.
 *
