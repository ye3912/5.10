...
#ifdef OPLUS_FEATURE_GPU_MINIDUMP
...
#ifdef OPLUS_FEATURE_GPU_MINIDUMP
	if (context != NULL) {
		dev_err(device->dev, "fault=%s, pid=%d, processname=%s\n",
			kgsl_get_reason(device->snapshotfault, gmu_fault),
			context->proc_priv->pid, context->proc_priv->comm);

		memset(snapshot->snapshot_hashid, '\0',
			sizeof(snapshot->snapshot_hashid));
		scnprintf(snapshot->snapshot_hashid,
			sizeof(snapshot->snapshot_hashid), "%d@%s@%s",
			context->proc_priv->pid, context->proc_priv->comm,
			kgsl_get_reason(device->snapshotfault, gmu_fault));
	}
#endif /* OPLUS_FEATURE_GPU_MINIDUMP */
...
#ifdef OPLUS_FEATURE_GPU_MINIDUMP
static bool snapshot_control_on;
	...
	size_t count)
{
	...
	if (snapshot_control_on) {
		dev_err(device->dev,
			"snapshot: snapshot_control_on is true, skip snapshot\n");
		return 0;
	}
#endif /* OPLUS_FEATURE_GPU_MINIDUMP */
...
#ifdef OPLUS_FEATURE_GPU_MINIDUMP
	WARN_ON(sysfs_create_file(&device->snapshot_kobj,
