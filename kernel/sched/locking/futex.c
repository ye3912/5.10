 ...
 *
 * Ported from 4.19: kernel/sched_assist/sync/futex.c
 * 5.10 adaptations:
 *  - Removed LINUX_VERSION_CODE guards
 *  - sched.h include uses relative path from kernel/sched/locking/
 *  - task_struct lkinfo/ux_state fields guarded by
 *    CONFIG_OPLUS_FEATURE_SCHED_ASSIST (available after separate port)
 */
				     ...
				     struct task_struct *p)
{
	...
	if (set_ux) {
		...
		if (type == UX_STATE_NONE) {
			...
			set_inherit_ux(holder, INHERIT_UX_FUTEX,
				       sa_ux_depth(p), sa_ux_state(p));
...
static void futex_unset_inherit_ux_refs(struct task_struct *p,
					int value, int path)
{
	bool clear = false;
	...
	cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
		...
		p->comm, p->pid, p->tgid, sa_inherit_ux(p), clear, path);
}
...
static bool boost_holder(struct task_struct *holder,
			 struct task_struct *waiter)
{
	/*
	 * When futex-opt disabled, still keep futex-holder tracing, it won't
	 * affect performance/power. But stop these two functions:
	 * 1. modify ux thread's pnode-prio in android_vh_alter_futex_plist_add()
	 * 2. set inherit ux thread with futex type in android_vh_futex_sleep_start()
	 */
	if (unlikely(!locking_opt_enable(LK_FUTEX_ENABLE)))
		return false;

	/*
	 * If current(ux thread) upgrade it's holder to inherit ux
	 * successfully, mark ux_contrib as true.
	 */
	if (futex_set_inherit_ux_refs(holder, waiter)) {
		sa_lkinfo(waiter).ux_contrib = true;
...
static bool set_holder(struct task_struct *waiter, struct task_struct *holder)
{
	if (waiter->tgid == holder->tgid) {
		/*
		 * If waiter's holder is set, do put_task_struct(holder)
		 * in futex_wait_end()
		 */
		sa_lkinfo(waiter).holder = holder;
		return true;
	}
...
void locking_vh_do_futex(int cmd, unsigned int *flags, u32 __user *uaddr2)
{
	...
	case FUTEX_WAIT_BITSET:
		do_something = (NULL != uaddr2) &&
			(thread_info_ctrl || curr_is_ux_thread_nolimit());
		if (do_something) {
			...
			if ((info.cmd & MAGIC_MASK) == MAGIC_NUM) {
				...
				if (info.cmd & OWNER_BIT) {
					/*
					 * owner_tid != 0 indicate that there
					 * is an owner to be set.
					 */
					if (info.owner_tid > 0 &&
						...
						*flags += info.owner_tid << FLAGS_OWNER_SHIFT;
						cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
							...
							current->tgid, info.owner_tid, *flags);
					}
				}
				...
				if (info.cmd & THREAD_INFO_BIT) {
					...
					if (leader &&
					    (sa_ux_im_flag(leader) == IM_FLAG_SYSTEMSERVER_PID))
					...
					if (copy_to_user(uaddr2, &info,
							 sizeof(struct futex_uinfo))) {
...
void locking_vh_futex_wait_start(unsigned int flags, u32 bitset)
{
	...
	holder = futex_find_task_by_pid(
		(flags & ~(0x3 << LOCK_TYPE_SHIFT)) >> FLAGS_OWNER_SHIFT);
	if (!holder)
		return;
...
#endif
	if (sa_lkinfo(current).holder) {
		if (unlikely(sa_lkinfo(current).ux_contrib)) {
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
				"curr(%-12s pid=%d tgid=%d) has woken, but ux_contrib is true! holder(%-12s pid=%d tgid=%d inherit_ux=%llx)\n",
				current->comm, current->pid, current->tgid,
				sa_lkinfo(current).holder->comm,
				sa_lkinfo(current).holder->pid,
				sa_lkinfo(current).holder->tgid,
				sa_inherit_ux(sa_lkinfo(current).holder));
			futex_unset_inherit_ux_refs(sa_lkinfo(current).holder, 1, 3);
		}

		put_task_struct(sa_lkinfo(current).holder);
		sa_lkinfo(current).holder = NULL;
	}
}
		    ...
		    struct plist_head *head, bool *already_on_hb)
{
	...
	holder = sa_lkinfo(current).holder;
	if (holder)
		boost_holder(holder, current);

	cur = (struct futex_q *) node;
	/*
	 * Find out the first normal thread in &hb->chain,
	...
	if (sleep_start) {
		...
		if (delta > FUTEX_WAITER_TOLERATE_THRESHOLD) {
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
				...
				first_normal_waiter->comm,
				first_normal_waiter->pid,
...
re_init:
	...
	cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
		...
		current->comm, current->pid, current->tgid, sa_ux_state(current),
		  ...
		  int *target_nr, union futex_key key, u32 bitset)
{
	...
	*target_nr = 0;
	...
	/*
	 * If waker is an inherit-futex ux thread, see how many
	 * ux waiters in this chain, make sure &hb->lock is held.
	 */
	plist_for_each_entry_safe(this, next, chain, list) {
		if (match_futex(&this->key, &key)) {
			p = this->task;

			if ((sa_lkinfo(p).holder == current) &&
			    sa_lkinfo(p).ux_contrib) {
				*target_nr += 1;
				sa_lkinfo(p).ux_contrib = false;
			}
			cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
...
void locking_vh_futex_wake_this(int ret, int nr_wake, int target_nr,
		  struct task_struct *p)
{
	if (sa_lkinfo(p).ux_contrib && sa_lkinfo(p).holder) {
		futex_unset_inherit_ux_refs(sa_lkinfo(p).holder, 1, 1);
		sa_lkinfo(p).ux_contrib = false;
	}
...
void locking_vh_futex_wake_up_q_finish(int nr_wake, int target_nr)
{
	...
	if (target_nr) {
		...
		cond_trace_printk(locking_opt_debug(LK_DEBUG_FTRACE),
			...
			current->comm, current->pid, current->tgid,
			target_nr, sa_inherit_ux(current));
	}
}

void init_task_lkinfo(struct task_struct *p)
{
	sa_lkinfo(p).waittime_stamp = 0;
	sa_lkinfo(p).opt_spin_start_time = 0;
	sa_lkinfo(p).holder = NULL;
	sa_lkinfo(p).ux_contrib = false;
	sa_lkinfo(p).is_block_ux = false;
}
