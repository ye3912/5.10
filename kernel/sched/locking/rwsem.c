 ...
 *
 * Ported from 4.19: kernel/sched_assist/sync/rwsem.c
 * 5.10 adaptations:
 *  - Removed LINUX_VERSION_CODE guards (always 5.10 path)
 *  - Always include trace/hooks/dtask.h (5.10 standard)
 */
...
bool rwsem_list_add(struct task_struct *tsk, struct list_head *entry,
		    struct list_head *head, struct rw_semaphore *sem)
{
...
void rwsem_set_inherit_ux(struct task_struct *tsk,
		struct task_struct *waiter_task, struct task_struct *owner,
	...
	if (waiter_task && is_ux) {
		if (rwsem_owner_is_writer(owner) && sem && !sem->ux_dep_task) {
			...
			if ((UX_STATE_NONE == type) ||
			    (UX_STATE_INHERIT == type)) {
			set_inherit_ux(owner, INHERIT_UX_RWSEM,
				       sa_ux_depth(tsk), sa_ux_state(tsk));
...
void locking_vh_rwsem_opt_spin_start(struct rw_semaphore *sem,
		bool *time_out, int *cnt, bool chk_only)
{
	u64 delta;
	...
	if (((sa_ux_state(current) & 0xf) &&
	     delta > rwsem_ux_opt_spin_time_threshold) ||
	    (!(sa_ux_state(current) & 0xf) &&
	     delta > rwsem_opt_spin_time_threshold)) {
		rwsem_opt_spin_timeout_exit_cnt++;
		*time_out = true;
	}
}

void locking_vh_rwsem_opt_spin_finish(struct rw_semaphore *sem,
		bool taken, bool wlock)
{
...
void locking_vh_rwsem_can_spin_on_owner(struct rw_semaphore *sem,
		bool *ret, bool wlock)
{
	struct oplus_rw_semaphore *osem = get_oplus_rw_semaphore(sem);
	long block_ux_cnt;
	...
	if (wlock || (sa_ux_state(current) & 0xf) || (current->prio < MAX_RT_PRIO))
		return;

	block_ux_cnt = atomic_long_read(&osem->count);
	/*
	 * If some ux task is in the waiter list,
