...
/*
 * OPLUS_FEATURE_SCHED_ASSIST — UX load tracking for frequency policy
 */
...
/*
 * OPLUS_FEATURE_POWER_CPUFREQ / POWER_EFFICIENCY — SchedTune integration
 *
 * schedtune_window_policy() needs only POWER_CPUFREQ (update_history §8).
 * The three uclamp_* functions additionally require POWER_EFFICIENCY
 * (is_ed_task §2, update_top_tasks §6, account_busy_for_task_demand §7).
 */
...
/*
 * OPLUS_FEATURE_EDTASK_IMPROVE — Early detection task migration
 */
#ifdef CONFIG_OPLUS_FEATURE_EDTASK_IMPROVE
extern int sysctl_ed_task_enabled;
#endif /* CONFIG_OPLUS_FEATURE_EDTASK_IMPROVE */
...
#if defined(CONFIG_OPLUS_FEATURE_POWER_CPUFREQ) && \
	defined(CONFIG_OPLUS_FEATURE_POWER_EFFICIENCY)
...
#ifdef CONFIG_OPLUS_FEATURE_SCHED_ASSIST
	{
		u64 oplus_wallclock = walt_ktime_get_ns();
		u64 timeline = 0;
		int cpu = cpu_of(rq);

		if (sched_assist_scene(SA_SLIDE) && ux_load_ts[cpu]) {
			timeline = oplus_wallclock - ux_load_ts[cpu];
...
#ifdef CONFIG_OPLUS_FEATURE_EDTASK_IMPROVE
/*
 * migrate_ed_task - Reset early detection state on task migration
 * to a faster CPU, so the ed task is re-evaluated on the new CPU.
 */
static void migrate_ed_task(struct task_struct *p, u64 wallclock,
			    struct rq *src_rq, struct rq *dest_rq)
{
	...
	if (capacity_orig_of(src_cpu) < capacity_orig_of(dest_cpu)) {
		...
		if (dest_wrq->ed_task == p)
			dest_wrq->ed_task = NULL;
	}
}
#endif /* CONFIG_OPLUS_FEATURE_EDTASK_IMPROVE */
			 ...
			 u32 runtime, int samples, int event)
{
	...
	/*
	 * NOTE: 'ld_flag' must be added to struct walt_task_struct
	 * when porting SCHED_SPREAD. Keep CONFIG_OPLUS_FEATURE_SCHED_SPREAD=n
	 * until the supporting sched_assist code is ported.
	 */
	if (p == rq->curr && p == current && event != PUT_PREV_TASK &&
	    p->sched_class == &fair_sched_class && sa_ld_flag(p))
