...
{
	if (fbg_state(ots) & FRAME_COMPOSITION)
		return &sf_composition_group;
	else if (fbg_state(ots) & FRAME_GAME)
...
static void remove_task_from_frame_group(struct task_struct *tsk)
{
	...
	if (fbg_state(tsk) & STATIC_FRAME_TASK) {
		list_del_init(&fbg_list(tsk));
		fbg_state(tsk) = NONE_FRAME_TASK;
		fbg_depth(tsk) = INVALID_FBG_DEPTH;
...
static void clear_all_static_frame_task(struct frame_group *grp)
{
	...
	list_for_each_entry_safe(p, tmp, &grp->tasks, fbg_list) {
		if (fbg_state(p) & STATIC_FRAME_TASK) {
			list_del_init(&fbg_list(p));
			fbg_state(p) = NONE_FRAME_TASK;
			fbg_depth(p) = INVALID_FBG_DEPTH;
...
{
	if (fbg_state(task) || task->flags & PF_EXITING)
		return;

	list_add(&fbg_list(task), &grp->tasks);
	fbg_state(task) = STATIC_FRAME_TASK;

	if (grp == &sf_composition_group)
		fbg_state(task) |= FRAME_COMPOSITION;
	else if (grp == &game_frame_boost_group)
		fbg_state(task) |= FRAME_GAME;
...
static void remove_binder_from_frame_group(struct task_struct *binder)
{
	...
	if (!(fbg_state(binder) & BINDER_FRAME_TASK))
		return;

	grp = task_get_frame_group(binder);
	frame_grp_with_lock_assert(grp);

	list_del_init(&fbg_list(binder));
	fbg_state(binder) = NONE_FRAME_TASK;
	fbg_depth(binder) = INVALID_FBG_DEPTH;
...
static void add_binder_to_frame_group(struct task_struct *binder, struct task_struct *from)
{
	...
	/* game_frame_boost_group not add binder task */
	if (fbg_state(from) & FRAME_GAME)
		return;

	if (fbg_state(from) & FRAME_COMPOSITION) {
	...
	if (fbg_state(from) == NONE_FRAME_TASK || fbg_state(binder))
		goto unlock;

	if (grp->binder_thread_num > MAX_BINDER_THREADS)
		goto unlock;

	if ((fbg_state(from) & BINDER_FRAME_TASK) &&
		fbg_depth(from) >= DEFAULT_BINDER_DEPTH) {
		goto unlock;
	}

	get_task_struct(binder);
	list_add(&fbg_list(binder), &grp->tasks);
	fbg_state(binder) = BINDER_FRAME_TASK;

	if (composition_part)
		fbg_state(binder) |= FRAME_COMPOSITION;

	fbg_depth(binder) = fbg_depth(from) + 1;
	grp->binder_thread_num++;
 ...
 */
void task_rename_hook(struct task_struct *p, const char *buf)
{
}
 ...
 *       the other paramenter is unused
 */
void fbg_binder_wakeup_hook(struct task_struct *caller_task,
 ...
 *       the other paramenter is unused
 */
void fbg_binder_restore_priority_hook(struct binder_transaction *t,
 ...
 *       the other paramenter is unused
 */
void fbg_binder_wait_for_work_hook(bool do_proc_work,
...
void fbg_sync_txn_recvd_hook(struct task_struct *tsk, struct task_struct *from)
{
	...
	bool need_freq_update)
{
	...
	if (fbg_state(tsk) == NONE_FRAME_TASK)
		return;

	if (fbg_state(tsk) & FRAME_COMPOSITION) {
		composition_part = true;
		grp = &sf_composition_group;
		lock = &sf_fbg_lock;
	} else if (fbg_state(tsk) & FRAME_GAME) {
		grp = &game_frame_boost_group;
		lock = &game_fbg_lock;
	} else {
		default_part = true;
...
void fbg_update_cfs_util_hook(struct task_struct *tsk,
	u64 runtime, u64 vruntime)
{
	fbg_update_task_util(tsk, runtime, true);
}

void fbg_update_rt_util_hook(struct task_struct *tsk, u64 runtime)
{
	fbg_update_task_util(tsk, runtime, true);
}
...
static void update_group_nr_running(struct task_struct *p, int event)
{
	...
	raw_spin_lock_irqsave(lock, flags);
	if (event == PICK_NEXT_TASK && (fbg_state(p) & STATIC_FRAME_TASK)) {
		fbg_running(p) = true;
		grp->nr_running++;
		if (grp->nr_running == 1)
	...
	} else if (event == PUT_PREV_TASK && fbg_running(p)) {
		fbg_running(p) = false;
		grp->nr_running--;
		if (unlikely(grp->nr_running < 0))
			grp->nr_running = 0;
	...
	struct task_struct *next, struct rq *rq)
{
	...
	if (unlikely(sysctl_frame_boost_debug) && next) {
		fbg_state_systrace_c(cpu_of(rq), fbg_state(next));
	}
...
bool set_frame_group_task_to_perfer_cpu(struct task_struct *p, int *target_cpu)
{
	...
	if (fbg_list(p).prev == 0 && fbg_list(p).next == 0) {
		fbg_state(p) = NONE_FRAME_TASK;
		fbg_depth(p) = INVALID_FBG_DEPTH;
		INIT_LIST_HEAD(&fbg_list(p));
	}
...
retry:
	...
	for_each_cpu(iter_cpu, &search_cpus) {
		...
		if (curr) {
			...
			if (sa_ux_state(curr) & (SCHED_ASSIST_UX_MASK | POSSIBLE_UX_MASK | SA_TYPE_INHERIT))
				continue;
...
bool fbg_need_up_migration(struct task_struct *p, struct rq *rq)
{
	...
	if (!fbg_state(p) || fbg_state(p) & (FRAME_COMPOSITION | FRAME_GAME))
		return false;
...
bool fbg_skip_migration(struct task_struct *tsk, int src_cpu, int dst_cpu)
{
	...
	if (!fbg_state(tsk) || fbg_state(tsk) & (FRAME_COMPOSITION | FRAME_GAME))
		return false;

	if (fbg_state(dst_rq->curr))
		return true;
...
bool fbg_rt_task_fits_capacity(struct task_struct *tsk, int cpu)
{
	...
	/* Some threads created before moduler working, just init them here. */
	if (fbg_list(tsk).prev == 0 && fbg_list(tsk).next == 0) {
		fbg_state(tsk) = NONE_FRAME_TASK;
		fbg_depth(tsk) = INVALID_FBG_DEPTH;
		INIT_LIST_HEAD(&fbg_list(tsk));
	}
 ...
 *********************************/
void fbg_flush_task_hook(struct task_struct *tsk)
{
	unsigned long flags;
	raw_spinlock_t *lock = NULL;
...
void fbg_sched_fork_hook(struct task_struct *tsk)
{
	fbg_state(tsk) = NONE_FRAME_TASK;
	fbg_depth(tsk) = INVALID_FBG_DEPTH;
	fbg_running(tsk) = false;
	fbg_preferred_cluster_id(tsk) = -1;
	INIT_LIST_HEAD(&fbg_list(tsk));
}
...
int info_show(struct seq_file *m, void *v)
{
	...
	list_for_each_entry(tsk, &grp->tasks, fbg_list) {
		seq_printf(m, "comm=%-16s  pid=%-6d  tgid=%-6d  state=%d  depth=%d\n",
			tsk->comm, tsk->pid, tsk->tgid, fbg_state(tsk), fbg_depth(tsk));
	}
	raw_spin_unlock_irqrestore(&def_fbg_lock, flags);
