 ...
 *
 * sched_assist common header — ported from 4.19 to 5.10.
 *
 * 5.10 adaptations:
 *  - Removed LINUX_VERSION_CODE guards (always 5.10)
 *  - Inline accessors that touch task_struct OPLUS fields guarded by
 *    CONFIG_OPLUS_FEATURE_SCHED_ASSIST (fields added in separate port step)
 */
...
/*
 * 5.10 GKI: Inline accessors use sa_wts()/sa_wrq() macros to reach
 * sched_assist fields inside walt_task_struct / walt_rq, which overlay
 * task_struct->android_vendor_data1 and rq->android_vendor_data1.
 *
 * In 4.19, code accessed task->ux_state directly. In 5.10, use
 * sa_ux_state(task) which expands to sa_wts(task)->ux_state.
 *
 * Without CONFIG_OPLUS_FEATURE_SCHED_ASSIST these are no-ops.
 */
...
{
	return sa_ux_state(task) & SA_TYPE_HEAVY;
}
...
static inline bool is_heavy_ux_task(struct task_struct *task) { return false; }
static inline void set_once_ux(struct task_struct *task) {}
static inline void set_heavy_ux(struct task_struct *task) {}
...
#else
static inline bool sched_assist_scene(unsigned int scene) { return false; }
#endif
...
{
	return sa_oplus_task_info(task).im_small;
}
#else
static inline bool is_small_task(struct task_struct *task) { return false; }
#endif
#endif
