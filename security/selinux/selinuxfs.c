...
/**
 * sel_get_enforce() - Return SELinux enforcing state to kernel callers
 *
 * Return: 1 if SELinux is enforcing, 0 if permissive.
 */
...
/**
 * sel_set_enforce() - Switch SELinux enforcing/permissive state from kernel
 * @new_value: 1 = enforcing, 0 = permissive
 *
 * This is a kernel-space equivalent of sel_write_enforce(). It allows
 * in-kernel code to temporarily drop SELinux to permissive mode without
 * going through the /sys/fs/selinux/enforce interface.
 *
 * Must be called from process context. Adapted for 5.10 which uses
 * call_blocking_lsm_notifier() instead of 4.19's call_lsm_notifier().
 *
 * Originally from libxzr/kernel-playground commit 1afe0d1.
 */
void sel_set_enforce(int new_value)
{
	struct selinux_state *state = &selinux_state;
	int old_value = enforcing_enabled(state);
...
EXPORT_SYMBOL_GPL(sel_set_enforce);
